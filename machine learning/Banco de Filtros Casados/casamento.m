clear;clc;close all;
%% Classificação por Casamento de Modelos
...
for j = 1 : quantIm
    imtrain=double(imread(strcat('../base de dados/train/com/',arquivos_c(j+2).name)));
    imtrain=imtrain(:);
    imtrain=imtrain-mean(imtrain);
    imtrain=imtrain/norm(imtrain);
    
    FiltCom=[FiltCom,imtrain];
    
    imtest=double(imread(strcat('../base de dados/train/sem/',arquivos_s(j+2).name)));
    imtest=imtest(:);
    imtest=imtest-mean(imtest);
    imtest=imtest/norm(imtest);
    
    FiltSem=[FiltSem,imtest];
end
...
for j = 1 : quantIm
    imtrain=double(imread(strcat('../base de dados/test/com/',arquivos_c(j+2).name)));
    imtrain=imtrain(:);
    imtrain=imtrain-mean(imtrain);
    imtrain=imtrain/norm(imtrain);
    
    dadosTest_com=[dadosTest_com,imtrain];
    
    imtest=double(imread(strcat('../base de dados/test/sem/',arquivos_s(j+2).name)));
    imtest=imtest(:);
    imtest=imtest-mean(imtest);
    imtest=imtest/norm(imtest);
    
    dadosTest_sem=[dadosTest_sem,imtest];
